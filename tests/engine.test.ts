...
function aggregate(count: number, homeOverrides = {}, awayOverrides = {}) {
  ...
  for (let seed = 1; seed <= count; seed += 1) {
    const result = simulateMatch({
      seed,
      neutralVenue: true,
      home: makeTeam("home", 10, homeOverrides),
      away: makeTeam("away", 10, awayOverrides),
    });
...
describe("Match Engine core", () => {
  it("replays the same seed exactly", () => {
    expect(simulateMatch(input(777))).toEqual(simulateMatch(input(777)));
  });

  it("changes output when the seed changes", () => {
    expect(simulateMatch(input(778)).events).not.toEqual(simulateMatch(input(777)).events);
  });
  ...
  it("produces a stable hash for a known match output", () => {
    const output = simulateMatch(input(424242));
    expect(stableHash(output)).toMatch(/^fnv1a64:[0-9a-f]{16}$/);
  });
});
...
describe("Behavioural presence", () => {
  it("makes each non-baseline formation statistically distinguishable", () => {
    ...
    for (const formation of alternatives) {
      ...
      expect(Math.abs(candidate.homeGoalsPerMatch - baseline.homeGoalsPerMatch)).toBeGreaterThanOrEqual(
        ENGINE_CONFIG.presenceTests.formationMinimumGoalRateDelta,
      );
    }
  });
  ...
  it("makes styles differ in chance volume and conversion independently", () => {
    ...
    for (const style of styles) {
      ...
      expect(Math.abs(candidate.chancesPerMatch - baseline.chancesPerMatch)).toBeGreaterThanOrEqual(
        ENGINE_CONFIG.presenceTests.styleMinimumChanceRateDelta,
      );
      expect(Math.abs(candidate.shotConversion - baseline.shotConversion)).toBeGreaterThanOrEqual(
        ENGINE_CONFIG.presenceTests.styleMinimumConversionDelta,
      );
    }
  });
  ...
  it("exercises every contribution field used by the ratings ledger", () => {
    ...
    for (const [field, total] of Object.entries(coverage)) {
      expect(total, `${field} was never exercised`).toBeGreaterThan(0);
    }
  });
});
