...
describe("attribute value evidence", () => {
  ...
  it("proves the zero control ran for the real treated team rather than accepting a stub", () => {
    expect(getZeroControlEvidence().zeroTreatment.teamId).toBe("sweep-12-team-10");
  }, 120_000);

  it("proves the zero control ran every supplied season rather than accepting a stub", () => {
    expect(getZeroControlEvidence().zeroTreatment.seasons).toBe(evidenceSeasonNumbers.length);
  }, 120_000);
