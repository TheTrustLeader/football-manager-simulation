// Frozen pre-optimisation reference used only by the REVIEW-002 equivalence harness.
// Remove after the structural optimisation evidence is accepted and the golden hash is re-recorded.
...
export function simulateMatchReference(input: MatchInput): MatchOutput {
  validateMatchInputReference(input);
  const c = ENGINE_CONFIG;
...
export function validateMatchInputReference(input: MatchInput): void {
  for (const team of [input.home, input.away]) {
