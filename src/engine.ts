...
export function validateMatchInput(input: MatchInput): void {
  ...
  for (const decision of input.decisions ?? []) {
    ...
    const team = decision.teamId === input.home.id ? input.home : decision.teamId === input.away.id ? input.away : undefined;
    if (!team) throw new Error(`Unknown decision team: ${decision.teamId}`);
    const on = team.substitutes.find((player) => player.id === decision.playerOn);
    if (!on) throw new Error(`${decision.playerOn} is not an available substitute`);
  }
  for (const team of [input.home, input.away]) {
    ...
    const usedEntrants = new Set<string>();
    for (const decision of decisions.map((value, index) => ({ value, index })).sort((a, b) => a.value.minute - b.value.minute || a.index - b.index).map(({ value }) => value)) {
      if (!active.has(decision.playerOff)) throw new Error(`${decision.playerOff} is not on the pitch`);
      if (!available.has(decision.playerOn)) throw new Error(`${decision.playerOn} is not an available substitute`);
      if (usedEntrants.has(decision.playerOn)) throw new Error(`${decision.playerOn} cannot be brought on twice`);
      const off = [...team.starters, ...team.substitutes].find((player) => player.id === decision.playerOff)!;
      const on = team.substitutes.find((player) => player.id === decision.playerOn)!;
      if ((off.primaryPosition === "GK") !== (on.primaryPosition === "GK")) {
        throw new Error("A goalkeeper may only be replaced by a goalkeeper");
      }
      active.delete(decision.playerOff);
      active.add(decision.playerOn);
      available.delete(decision.playerOn);
      usedEntrants.add(decision.playerOn);
    }
  }
}
